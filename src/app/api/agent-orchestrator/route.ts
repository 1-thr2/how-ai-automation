...
## 🔄 **중복 방지 원칙** (매우 중요):
### ❌ **절대 반복하지 말 것**:
- **같은 도구의 로그인/회원가입**: 이전 단계에서 이미 로그인한 도구는 로그인 과정 생략
- **동일한 도구 접속**: 이미 접속한 사이트는 접속 과정 생략  
- **기본 설정**: 이미 생성된 계정이나 설정은 반복하지 않음

### ✅ **대신 이렇게 표현**:
- **이전 단계에서 접속한 [도구명]에서 이어서...**
- **로그인된 상태에서 다음 작업을 진행합니다**
- **앞서 사용한 [도구명]의 대시보드에서 새로운 작업을 시작합니다**

### 🎯 **이전 단계 연결성 체크**:
현재 ${step.id}번째 단계이므로:
${previousGuides.length > 0 ? 
`- 이전 단계들에서 사용한 도구: ${previousGuides.map(g => g.title).join(', ')}
- 이미 설정된 내용은 반복하지 말고 연결해서 설명` : 
