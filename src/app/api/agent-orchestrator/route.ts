...
### 🛠️ **선택 가능한 접근법들** (효율성 우선순위):
- **🏆 AI 도구 우선**: 빠르고 쉬운 AI 도구 (ChatGPT, Claude, Zapier AI, Make.com AI)
- **📱 쉬운 방법**: 기존 도구 활용 (Notion AI, 구글 워크스페이스)
- **🔧 중간 방법**: 프롬프트 엔지니어링 + 간단한 자동화
- **💻 고급 방법**: 개발/API 연동 (필요시에만)
- **🌏 특화 방법**: 한국 도구, 기업 환경 맞춤

### 📋 **선택 기준** (우선순위 순):
1. **최적 효율성**: 가장 빠르고 쉬운 방법인지 (AI vs 수동 vs 코딩)
2. **통합 워크플로우**: 하나로 연결된 완전한 시스템 구성 가능성
3. **후속답변 적합성**: 사용자 성향과 환경에 맞는지
4. **실행 가능성**: 가장 효과적이면서 실현 가능한지
...
### ✅ **좋은 예시 (구체적)**:
- "1. 크롬 브라우저를 열고 주소창에 https://developers.naver.com/apps 입력 후 Enter
   2. 화면 오른쪽 상단의 파란색 '로그인' 버튼 클릭
   3. 네이버 아이디와 비밀번호 입력 후 로그인
   4. 로그인 후 화면 중앙의 초록색 '애플리케이션 등록' 버튼 클릭
   5. '애플리케이션 이름' 필드에 '내광고자동화' 입력
   6. 화면을 아래로 스크롤하여 '광고API' 체크박스에 체크
   7. 페이지 하단의 파란색 '등록하기' 버튼 클릭
   8. 등록 완료 후 나타나는 'Client ID' 값을 메모장에 복사해서 저장"

## 🔗 **연결된 워크플로우 설계 필수 원칙**:
1. **하나의 자동화 플랫폼**: 모든 단계가 하나의 Zapier/Make.com 워크플로우에서 연결
2. **명확한 데이터 흐름**: 이전 단계 출력 → 현재 단계 입력 → 다음 단계 입력의 구체적 경로
  ...
  "cards": [
    {
      ...
      "content": {
        ...
        "toolChoice": {
          "recommended": "추천 도구 (최적 효율성 우선, 통합 워크플로우 고려)",
          "reason": "왕초보도 따라할 수 있는 수준 + 최적 효율성 + 이전단계 연결성",
          "alternatives": [
            {
              "tool": "대안 도구",
              "pros": "장점",
