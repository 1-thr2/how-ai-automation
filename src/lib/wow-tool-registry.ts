...
export interface WowTool {
  name: string;
  type: 'slide_gen' | 'video_gen' | 'dashboard' | 'landing_gen' | 'creative_gen' | 'avatar_video' | 'audio_gen' | 'chatbot_gen' | 'workflow';
...
export function selectOptimalTool(userInput: string, followupAnswers?: any): WowTool {
  ...
  // 🎯 워크플로우 우선 판단 키워드
  const workflowKeywords = ['자동화', '연동', '업데이트', '알림', '모니터링', '수집', '전송', '스케줄', '매일', '실시간', '시트', 'slack', 'dm', 'roas', '성과', '데이터'];
  const hasWorkflowContext = workflowKeywords.some(keyword => 
    input.includes(keyword) || koreanKeywords.some(k => k.includes(keyword))
  );
  ...
  const toolScores = tools.map(tool => {
    ...
    const koreanMatchCount = koreanKeywords.filter(korKeyword => 
      tool.bestFor.some(toolKeyword => 
        korKeyword.includes(toolKeyword) || toolKeyword.includes(korKeyword)
      )
    ).length;
  ...
  console.log(`🔍 상위 3개 툴 스코어:`, toolScores
...
export function getTopWowTools(limit: number = 5): WowTool[] {
  return WOW_TOOL_REGISTRY
    .filter(tool => tool.wowScore >= 7)
