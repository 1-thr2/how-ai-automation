...
## 🔧 연동 가능성 확인 질문 (필요시에만!)
사용자가 특정 도구를 명시적으로 언급한 경우에만 다음과 같은 연동 확인 질문을 포함하세요:
...
반드시 유효한 JSON 배열 형식으로만 응답하세요. 마크다운 블록이나 다른 텍스트는 포함하지 마세요.

올바른 응답 예시:
[{"key": "data_source", "question": "현재 처리하는 데이터는 주로 어디에서 오나요?", "type": "single", "options": ["엑셀/구글시트", "데이터베이스", "웹사이트", "이메일", "기타 (직접입력)", "잘모름 (AI가 추천)"], "category": "data", "importance": "high", "description": "데이터 소스를 파악하여 최적의 연동 방법을 제안하기 위함", "inputTriggers": {"기타 (직접입력)": {"requiresInput": true, "inputPlaceholder": "어떤 데이터 소스를 사용하시는지 구체적으로 설명해주세요"}, "잘모름 (AI가 추천)": {"requiresInput": true, "inputPlaceholder": "현재 데이터 관리 상황을 간단히 설명해주세요"}}}]

잘못된 응답 형식: 마크다운 블록 사용하거나 객체로 감싸기
올바른 응답 형식: 배열 직접 반환`;
...
### 📋 질문 타입 설정 (기본값: multiple):
- **"multiple" (기본값)**: 여러 개 선택 가능 (플랫폼 목록, 도구 목록, 기능 목록 등)
- **"single"**: 하나만 선택 가능 (매우 제한적 사용 - 계정 보유 여부, Yes/No 등)

**❗ 중요: 99% 질문은 "multiple" 타입을 사용하세요!**

**✅ multiple 사용 케이스 (거의 모든 질문)**:
- 플랫폼/도구 선택: "어떤 플랫폼을 사용하고 계신가요?"
- 기능 요구사항: "어떤 기능이 필요하신가요?"  
- 데이터 유형: "어떤 종류의 데이터를 다루시나요?"
- 업무 프로세스: "현재 어떤 방식으로 작업하고 계신가요?"

**❌ single은 극히 제한적으로만 사용**:
- "개발자 계정이 있으신가요?" (Yes/No)
- "이 기능을 사용해본 적이 있으신가요?" (Yes/No)
...
입력이 필요한 선택지에는 inputTriggers 객체를 반드시 포함하세요.`;
...
- type: **"multiple" (기본값)** 또는 "single" (제한적 사용)
- options: 선택지 배열 (마지막에 "기타 (직접입력)", "잘모름 (AI가 추천)" 포함)
- inputTriggers: 입력창이 필요한 옵션들에 대한 설정

### 🎯 중요: 타입 설정 가이드 (기본값: multiple)
**❗ 99% 질문은 "multiple" 타입을 사용하세요!**

- **"multiple" (기본값)**: 이메일, 웹사이트, 슬랙, 메신저 앱, 카카오톡, 도구 선택, 기능 요구사항 등 **거의 모든 질문**
- **"single" (극히 제한적)**: 계정 보유 여부(Yes/No), 사용 경험 정도 등 **이분법적 선택만**
...
입력창이 필요한 모든 선택지에 inputTriggers 반드시 포함하세요.
...
올바른 형식:
{"cards": [{"type": "needs_analysis", "title": "🎯 니즈 분석", "surfaceRequest": "사용자가 말한 것", "realNeed": "실제 필요한 것", "recommendedLevel": "반자동", "status": "draft"}, {"type": "flow", "title": "🚀 자동화 플로우", "subtitle": "기본 단계별 계획", "steps": [{"id": "1", "title": "단계 1 제목", "tool": "추천 도구"}], "status": "draft"}]}
...
## 🔥 WOW! 즉시 활용 가능한 실전 솔루션

### ✅ 이런 수준의 WOW 팩터를 만들어주세요:

**🚀 창의적이고 실용적인 아이디어**:
- "어? 이런 방법이 있었구나!" (놀라움)
- "우리 회사에 딱 맞는 솔루션이네!" (적합성)
- "지금 당장 해보고 싶어!" (실행 욕구)
- "이거 하나로 엄청 많은 게 해결되겠다!" (임팩트)

**💎 구체적인 WOW 사례들**:
1. **단순 키워드 매칭 → AI 감정 분석 + VIP 고객 자동 인식**
   - 화난 고객 톤 감지 → 즉시 팀장에게 알림
   - 대기업 도메인 감지 → VIP 처리 프로세스 자동 시작

2. **일반적인 Slack 알림 → 게임화된 실시간 대시보드**
   - 긴급 문의 해결시 포인트 적립
   - 팀별 응답속도 리더보드 표시
   - 고객 만족도에 따른 배지 시스템
...
**🎯 사용자 맞춤형 창의성 (핵심!):**
❌ 무지성 창의성: 누구에게나 "게임화! 포인트 적립!"
✅ 맞춤형 창의성: 사용자 상황을 정확히 분석한 후 적합한 WOW 제안

**분석 기준:**
1. **팀 규모**: 소규모(효율성) vs 대규모(경쟁/협업)
2. **기술 수준**: 초보자(간단함) vs 고급자(혁신성)  
3. **예산**: 무료(창의적 활용) vs 유료(고급 기능)
4. **업무 스타일**: 협업형 vs 경쟁형 vs 개인형

**맞춤 창의성 예시:**
- 소규모 + 초보자 → "VIP 고객 자동 감지로 놓치는 고객 Zero"
- 대규모 + 경쟁형 → "팀별 응답속도 리더보드로 자연스런 서비스 향상"
- 기술팀 + 예산 → "AI 감정 분석으로 고객 화내기 전 사전 대응"

## 🚨 극도로 상세한 가이드 생성 (왕초보 기준)

### 🔥 WOW 임팩트를 위한 혁신적 사례들:

**💡 사례 1: 단순 알림 → 게임화된 팀 경쟁**
- ❌ 기존: "Slack에 문의 알림"
- ✅ WOW: "팀별 응답속도 실시간 순위표 + 포인트 적립 시스템"
  - 빠른 응답시 포인트 획득
  - 월말 1등팀 치킨 쿠폰 자동 지급
  - 고객 만족도 5점시 보너스 점수

**🚀 사례 2: 키워드 필터링 → AI 감정 + VIP 자동 인식**
- ❌ 기존: "긴급 키워드 감지"  
- ✅ WOW: "화난 고객 톤 + 대기업 도메인 자동 감지"
  - "실망스럽다", "답답하다" 감정 키워드 분석
  - @samsung.com, @lg.com 등 VIP 고객 자동 태깅
  - 화난 VIP 고객 = 즉시 팀장 호출 + 30분 내 응답 필수

**💎 사례 3: 데이터 입력 → 전사 인사이트 대시보드**
- ❌ 기존: "스프레드시트에 데이터 저장"
- ✅ WOW: "문의 패턴 분석 → 제품 개선 제안 자동 생성"
  - 같은 버그 신고 3회 이상 = 개발팀 긴급 알림
  - 기능 요청 빈도 분석 → 다음 업데이트 우선순위 제안
  - 고객 불만 키워드 트렌드 → 마케팅팀 대응 전략 자동 생성
...
- **codeBlocks 배열 (완벽한 복붙 가이드 필수!)**:
  각 블록은 title, language, code, copyInstructions, saveLocation 필드 포함
  예시: Zapier 필터 코드와 Slack 메시지 템플릿을 완전한 복붙 형태로 제공

- **stepByStepGuide 배열 (왕초보 완전정복 가이드)**:
  각 단계는 stepNumber, title, duration, actions, validation 필드 포함
  예시: "Zapier 계정 만들기", "구글 시트 연결하기" 등 구체적인 행동 지침
